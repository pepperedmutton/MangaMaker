...
const filterAllowedAgentModels = (models) => models
    ...
    return (isAllowedAgentModelProvider(model.id) &&
...
const getCurrentAgentConfig = async () => {
    ...
    if (!availableModels.some((entry) => entry.id === model)) {
        return {
            ...
            testMode: false,
            visionEnabled: false,
            reason: "Configured model is not available for MangaMaker Agent. Choose a DeepSeek or Kimi model with image input, text output, and JSON response_format support.",
        };
    }
    return {
        enabled: true,
...
const getHarnessToolResults = (harness) => [
...
const collectImageDataUrls = (value) => {
    ...
    const record = value;
    const ownDataUrl = typeof record.dataUrl === "string" && record.dataUrl.startsWith("data:image/")
        ? [record.dataUrl]
        : [];
    return [
        ...ownDataUrl,
...
const getHarnessImageDataUrls = (harness) => getHarnessToolResults(harness)
    .flatMap((entry) => collectImageDataUrls(entry.result))
    .slice(-MAX_AGENT_IMAGE_ATTACHMENTS);
const getRenderedPageToolResults = (payload) => getHarnessToolResults(payload.harness).filter((entry) => entry.toolName === "renderPage" || entry.toolName === "renderPages");
const hasExhaustedToolBudget = (payload) => getHarnessToolResults(payload.harness).some((entry) => {
    if (entry.toolName !== "toolBudget" || !entry.result || typeof entry.result !== "object") {
        return false;
    }
    return entry.result.exhausted === true;
});
...
const createTestAgentResponse = (payload) => {
    ...
    if (latest.includes("删除") || latest.includes("清空") || normalized.includes("delete") || normalized.includes("clear")) {
        ...
        const command = targetsPage && pageId
            ? {
    ...
    if (latest.includes("保存") || normalized.includes("save")) {
        return {
            ...
            pendingCommandPlan: createPlan("Save the current project", [{ commandId: "saveProject", payload: {}, reason: "Save project.", dangerLevel: "normal" }], false),
            usedVision: hasVisionInput,
        };
    }
    if (latest.includes("描边") || normalized.includes("stroke")) {
        ...
        return {
            ...
            pendingCommandPlan: textId && pageId
                ? createPlan("Update text stroke", [
                    {
    ...
    if (normalized.includes("two panels") || latest.includes("两个分镜") || latest.includes("涓や釜鍒嗛暅")) {
        return {
            ...
            pendingCommandPlan: pageId
                ? createPlan("Create two panels", [
                    {
    ...
    if (latest.includes("文字") || latest.includes("文本") || normalized.includes("text")) {
        ...
        return {
            ...
            pendingCommandPlan: pageId
                ? createPlan("Create one text object", [
                    {
...
const callOpenRouter = async (payload, includeImage) => {
...
const createDynamicAgentSchemaLoader = () => async () => {
    const moduleName = pathToFileURL(path.resolve(process.cwd(), "src/agent/agentResponseSchema.ts")).href;
    return (await import(moduleName));
};
const normalizeAgentModelResponse = async (value, loadAgentSchema) => {
    const module = await loadAgentSchema();
    return module.validateAgentChatResponse(value);
};
const attachWebAgentMiddleware = (middlewares, loadAgentSchema = createDynamicAgentSchemaLoader()) => {
...
const webAgentPlugin = () => ({
    ...
    configureServer(server) {
        attachWebAgentMiddleware(server.middlewares, async () => server.ssrLoadModule("/src/agent/agentResponseSchema.ts"));
    },
    configurePreviewServer(server) {
