...
This product Agent is separate from external coding-agent workflows, but its internal harness follows the same basic shape: the Agent receives a tool catalog and a lightweight audited index, then chooses which project details to inspect. The initial prompt contains only project summary, page index, current-page marker, selection summary, and tool definitions. It does not preload every page object, every image resource, or the current screenshot. The Agent can call `searchProject`, `readPage`, `readPages`, `listImageAssets`, `renderPage`, `renderPages`, and `listCommandManifest` as needed. When it requests `renderPage` or `renderPages`, it receives the requested page resources and composed visual screenshots. Repeated identical tool requests are skipped, and a bounded tool budget forces the Agent to answer from gathered evidence rather than looping indefinitely. It uses the command manifest as its API reference. It only mutates the project through the local command registry, validates command payloads with the existing Zod schemas, and executes approved plans through the editor undo/redo history.
...
The Agent should assist with critique, planning, consistency checks, and mechanical editor operations. Story direction, page composition, final dialogue, and artistic judgment remain under the human creator's control. Large image data is not dumped into the initial prompt; visual renders and assets are exposed only through bounded, on-demand tool results or resource references.
...
Agent chat history is project-scoped and persistent by default. When a creator opens the Agent for the same project, previous messages are restored until the creator explicitly uses `Delete chat` in the Agent sidebar. Deleting chat history clears only that project's Agent conversation; it does not delete pages, resources, or project content. The durable record is written as `agent-chat.json` inside the project folder, for example `projects/<project-folder>/agent-chat.json`, so local debugging agents can read the active session without scraping browser `localStorage`.

The web/Vite backend provides `GET /__mangamaker__/agent/config`, `GET /__mangamaker__/agent/models`, `POST /__mangamaker__/agent/chat`, and `GET/POST/DELETE /__mangamaker__/agent/history?projectId=...`. While the sidebar is open, `GET /__mangamaker__/agent/debug` exposes a sanitized live snapshot for debugging stuck tool calls; the same snapshot is available in the browser through `window.mangaMaker.agent.getDebugSnapshot()`. The debug snapshot must not include API keys or base64 screenshot data.
