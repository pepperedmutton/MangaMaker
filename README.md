...
- `src/sysml`: SysML v2 Pilot adapter, project SysML repository, and MBSE seed models
...
This product Agent is separate from external coding-agent workflows, but its internal harness follows the same basic shape: the Agent receives a tool catalog and a lightweight audited index, then chooses which project details to inspect. The initial prompt contains only project summary, page index, current-page marker, selection summary, active role, active role metadoc id, active role metadoc content, active document id, compact SysML standard overview, and tool definitions. It does not preload every page object, every image resource, every Markdown document, every SysML file, or the current screenshot. The Agent can call `searchProject`, `readPage`, `readPages`, `listImageAssets`, `renderPage`, `renderPages`, `listRoles`, `listDocuments`, `readDocument`, `searchDocuments`, `writeDocument`, and `listCommandManifest` as needed. When it requests `renderPage` or `renderPages`, it receives the requested page resources and composed visual screenshots. Repeated identical tool requests are skipped, and a bounded tool budget pauses the run for a creator Continue/Stop decision instead of forcing an answer from incomplete evidence. It uses the command manifest as its API reference. It only mutates comic pages through the local command registry, validates command payloads with the existing Zod schemas, and executes approved plans through the editor undo/redo history.

For SysML-first workflows, the Agent harness now includes a built-in SysML v2/KerML/Pilot reference layer. `readSysmlStandardOverview` is preloaded into every Agent turn as a compact rule index, and the Agent can call `readSysmlStandardReference` to inspect focused topics such as packages/imports, definitions/usages, parts, requirements, behavior, views, MangaMaker MBSE mapping, and Pilot validation workflow. The Agent can then call `getSysmlStatus`, `listSysmlFiles`, `readSysmlFile`, `writeSysmlFile`, and `validateSysmlModel`. It must not load every SysML file by default; it lists and reads only the model files needed for the task, writes durable model changes through `writeSysmlFile`, and validates with the official SysML v2 Pilot before claiming the MBSE model is valid.

MangaMaker treats chat as conversation, not as the durable production source of truth. Each project owns Markdown documents under `projects/<project-folder>/docs/`, with a document and role manifest in `docs/manifest.json`. The center workspace can switch between `Comic` and `Docs`; the right Agent sidebar remains available in both modes. The left overview remains page-only and shows comic page thumbnails, not document overviews. Opening `Docs` first shows a file-viewer list of Markdown file names. Selecting a file opens a rendered Markdown viewer; raw Markdown editing is an explicit secondary action from that viewer. Right-click in the Docs workspace to create a new Markdown document, or right-click any document file to rename or delete it.

The center workspace can also switch to `SysML` for MBSE-grade project control. Each project owns SysML/KerML files under `projects/<project-folder>/sysml/`. New repositories are seeded with `mangamaker-domain.sysml` and `project-model.sysml`, modeling MangaMaker projects, production documents, comic pages, panels, image resources, text blocks, bubbles, Agent roles, and formal requirements. The web/Vite backend validates these files through the official SysML v2 Pilot Implementation when the local runtime is configured with `pnpm setup:sysml`. SysML is the formal engineering constraint model; Markdown remains the human-readable production record.

Every active Agent role is bound to exactly one metadoc, and each project may have only one active conversation session for each role. That metadoc defines the role and records that role's durable output. Default role metadocs include `Assistant Metadoc`, `Production Plan`, `Story Architecture`, `Storyboard Overview`, `Script and Dialogue`, `Art Supervision`, `Continuity Check`, and `Image Prompts`; they are starter files, and user deletion is respected rather than silently recreated. If a metadoc is deleted, its role disappears. If a role is deleted from the Agent sidebar, its metadoc is kept as an ordinary Markdown document. Ordinary documents do not have to be bound to any role, and the creator can create a new role either with an automatically generated metadoc or by choosing an existing ordinary document as that role's metadoc. Role modes such as Producer, Director, Storyboard Designer, Script Designer, Art Supervisor, Continuity Supervisor, and Prompt Engineer are expected to read, write, and execute against documents. In this context, "execute" means turning documented intent into bounded command plans or document updates, not taking over creative authorship.
...
The `writeDocument` harness tool is idempotent. Every model-requested document write must include an `operationId` for that exact document content. Reusing an `operationId` with the same content is treated as a retry; reusing it with different content is rejected.

The web/Vite backend provides `GET /__mangamaker__/agent/config`, `GET /__mangamaker__/agent/models`, `POST /__mangamaker__/agent/runs`, `GET /__mangamaker__/agent/runs/:runId?projectId=...`, `GET /__mangamaker__/agent/runs/:runId/events`, `POST /__mangamaker__/agent/runs/:runId/tool-results`, legacy `POST /__mangamaker__/agent/chat`, `GET/POST/DELETE /__mangamaker__/agent/conversation-context?projectId=...&roleId=...`, `GET /__mangamaker__/agent/documents?projectId=...`, `GET /__mangamaker__/agent/document?projectId=...&documentId=...`, `POST /__mangamaker__/agent/document`, `DELETE /__mangamaker__/agent/document`, `POST /__mangamaker__/agent/role`, and `DELETE /__mangamaker__/agent/role`. It also provides `GET /__mangamaker__/sysml/config`, `GET /__mangamaker__/sysml/files?projectId=...`, `GET /__mangamaker__/sysml/file?projectId=...&path=...`, `POST /__mangamaker__/sysml/file`, `DELETE /__mangamaker__/sysml/file`, and `POST /__mangamaker__/sysml/validate`. While the sidebar is open, `GET /__mangamaker__/agent/debug` exposes a sanitized live snapshot for debugging stuck tool calls and includes the active run id/status; the same snapshot is available in the browser through `window.mangaMaker.agent.getDebugSnapshot()`. The debug snapshot must not include API keys or base64 screenshot data.
