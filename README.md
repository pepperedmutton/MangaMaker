...
- `src/agent`: built-in creator assistance Agent config, harness, roles, documents, and validation
...
MangaMaker includes an in-product creator assistance Agent in the editor sidebar. Manga creation remains the creator's work: the Agent is there to inspect the current page, offer suggestions, explain options, and maintain project documents. It must not present itself as the author of the comic or take over end-to-end manga creation. In the current product scope, the built-in Agent does not directly modify comic pages, panels, text objects, bubbles, layers, or project structure.

This product Agent is separate from external coding-agent workflows, but its internal harness follows the same basic shape: the Agent receives a tool catalog and a lightweight audited index, then chooses which project details to inspect. Every project has a required `docs/PrimeDirective.md`. It is preloaded as `readPrimeDirective` for every Agent role and defines the project form, creator intent, operating constraints, and conflict priority for that project. Every role has one metadoc and one working directory. The metadoc is the pinned role prompt and role definition; role work output belongs in ordinary Markdown documents under that role's working directory, for example `docs/work/producer/`, unless the creator names another document. Multimodal models receive project/page/resource tools on demand. Text-only document models such as `deepseek/deepseek-v4-pro` receive a separate prompt and tool catalog: they can read the Prime Directive, the active role metadoc, and Markdown documents, but cannot inspect pages/images/renders and must ask the creator to switch to a multimodal model for visual work. The initial prompt pins only system instructions, Prime Directive, active role metadoc, active role metadata, active working directory, active document id, and tool definitions. It does not preload every page object, every image resource, every Markdown document, or the current screenshot. The Agent can call `searchProject`, `readPage`, `readPages`, `listImageAssets`, `renderPage`, `renderPages`, `listRoles`, `listDocuments`, `readDocument`, `searchDocuments`, `readDocumentLines`, `editDocumentLines`, `appendDocument`, `replaceDocumentSection`, `replaceDocumentText`, and `writeDocument` as needed. `appendDocument` is for additive notes, `editDocumentLines` is for arbitrary line edits, `replaceDocumentSection` is for heading-based section updates, `replaceDocumentText` is for exact small replacements, and `writeDocument` is reserved for full-document creation or replacement. When a multimodal model requests `renderPage` or `renderPages`, it receives the requested page resources and composed visual screenshots. Repeated identical tool requests are skipped, and a bounded tool budget pauses the run for a creator Continue/Stop decision instead of forcing an answer from incomplete evidence. Page command manifests and executable page command plans are not exposed to the Agent; page-change requests should become document updates or manual instructions for the creator.

MangaMaker treats chat as conversation, not as the durable production source of truth. Each project owns Markdown documents under `projects/<project-folder>/docs/`, with a document and role manifest in `docs/manifest.json`. `docs/PrimeDirective.md` is mandatory for every project, is created for new and migrated projects, and cannot be deleted from the document API because every Agent role depends on it to understand whether the project is a manga, CG set, illustrated light novel, or another form. The center workspace can switch between `Comic` and `Docs`; the right Agent sidebar remains available in both modes. The left overview remains page-only and shows comic page thumbnails, not document overviews. Opening `Docs` first shows a file-viewer list of Markdown file names. Selecting a file opens a rendered Markdown viewer; raw Markdown editing is an explicit secondary action from that viewer. Right-click in the Docs workspace to create a new Markdown document, or right-click any document file to rename or delete it.

Every active Agent role is bound to exactly one metadoc, one working directory, and one conversation session in each project. The Prime Directive defines the whole project; the role metadoc is that role's prompt and definition; the working directory stores that role's durable output. Role metadocs live under `docs/roles/`, and the filename must match the role name, for example `docs/roles/Producer.md`. Creating a role asks for the role name, the metadoc source, and the working directory path. The metadoc can be generated automatically or bound from an ordinary document; the working directory is created under `docs/` and is where that role writes output documents. Edit the metadoc itself when you want to change the role prompt. Default role metadocs include `Assistant Metadoc`, `Production Plan`, `Story Architecture`, `Storyboard Overview`, `Script and Dialogue`, `Art Supervision`, `Continuity Check`, and `Image Prompts`; they are starter role prompt/definition files, and user deletion is respected rather than silently recreated. If a metadoc is deleted, its role disappears. If a role is deleted from the Agent sidebar, its metadoc is kept as an ordinary Markdown document. Ordinary documents do not have to be bound to any role, and the creator can create a new role either with an automatically generated metadoc or by choosing an existing ordinary document as that role's metadoc. Role modes such as Producer, Director, Storyboard Designer, Script Designer, Art Supervisor, Continuity Supervisor, and Prompt Engineer are expected to read and write documents under the project-level constraints in `PrimeDirective.md`. In this context, "execute" means persisting durable document updates, not taking over creative authorship or directly editing comic pages.

Open the Agent from the top-right `Inspector` / `Agent` sidebar toggle, not from the editor ribbon. The sidebar always shows its configuration state before chat is available:
...
The Agent sidebar also exposes a front-end `Agent Config` panel for manual harness control. The creator can edit the effective system prompt for future turns, set the context-window token budget used for prompt compaction, and edit the conversation context that will be sent to the model, including both creator messages and Agent replies. The app still appends non-optional protocol instructions so model responses remain JSON-validated, document writes remain tool-verified, and page command plans remain disabled.

Agent suggestions may include page-edit recommendations, but they are written as Markdown planning/instruction updates rather than executable page command plans. The creator performs page, panel, text, bubble, layer, and save operations through the normal editor UI.

The Agent should assist with critique, planning, consistency checks, document maintenance, and mechanical editor operations. Story direction, page composition, final dialogue, and artistic judgment remain under the human creator's control. Large image data and full document contents are not dumped into the initial prompt; visual renders, assets, and Markdown documents are exposed only through bounded, on-demand tool results or resource references.

To control multimodal token cost, the Agent guidance and harness require a structured-first workflow. It should search or read page resources before asking for screenshots, use `renderPages` only for small preview samples, and use `renderPage` with `crop` for local detail. Page render tools default to `detail="preview"` with a 768 px maximum edge; `detail="detail"` uses a 1280 px maximum edge and should be reserved for small text, faces, or fine line art. PNG versus JPG is not treated as a reliable token-saving switch; the preferred savings are fewer images, smaller pixel dimensions, and tighter crops.

Conversation Context is scoped by project and role, and is persistent by default. It is the single editable history/context surface for the active role: switching role switches to that role's one session, reopening the Agent restores the active role's context messages, and `Agent Config` lets the creator edit, add, delete, or `Clear context` entries before the next turn. Clearing context removes only the active role's Agent conversation context; it does not delete pages, resources, Markdown documents, other role sessions, or project content. The durable role-indexed store is written as `agent-conversation-context.json` inside the project folder, for example `projects/<project-folder>/agent-conversation-context.json`, so local debugging agents can read all role sessions without scraping browser `localStorage`. Existing single-session `agent-conversation-context.json` and `agent-chat.json` records are read as migration fallbacks for the Assistant role.

Tool call status is shown inline in the Agent conversation rather than as a separate Tool Log panel. Clearing Conversation Context clears the visible conversation and tool status entries without adding a synthetic delete event to the tool timeline.

Agent execution is persisted as an `agentRun`, not as one long blocking chat fetch. A run is stored under the project `agent-runs/` folder and is split into steps such as `model_request`, `tool_call`, `tool_result`, `model_resume`, `retry`, and `command_plan`. The sidebar subscribes to run status through Server-Sent Events, so long provider calls expose live state instead of leaving the UI waiting on a 120 second request. Provider timeouts, terminated upstream connections, HTTP 429, HTTP 5xx, non-JSON responses, and invalid Agent JSON are recorded in the run trace and retried with fallback OpenRouter routing when possible. The harness preserves the latest unique tool results in the prompt even when budget/skip status messages accumulate, and repeated identical tool calls are skipped and then paused if the model keeps repeating them.

Document mutation harness tools are idempotent. Every model-requested document write must include an `operationId` for that exact document edit. Reusing an `operationId` with the same edit is treated as a retry; reusing it with different content is rejected.

The web/Vite backend provides `GET /__mangamaker__/agent/config`, `GET /__mangamaker__/agent/models`, `POST /__mangamaker__/agent/runs`, `GET /__mangamaker__/agent/runs/:runId?projectId=...`, `GET /__mangamaker__/agent/runs/:runId/events`, `POST /__mangamaker__/agent/runs/:runId/tool-results`, legacy `POST /__mangamaker__/agent/chat`, `GET/POST/DELETE /__mangamaker__/agent/conversation-context?projectId=...&roleId=...`, `GET /__mangamaker__/agent/documents?projectId=...`, `GET /__mangamaker__/agent/document?projectId=...&documentId=...`, `POST /__mangamaker__/agent/document`, `DELETE /__mangamaker__/agent/document`, `POST /__mangamaker__/agent/role`, and `DELETE /__mangamaker__/agent/role`. While the sidebar is open, `GET /__mangamaker__/agent/debug` exposes a sanitized live snapshot for debugging stuck tool calls and includes the active run id/status; the same snapshot is available in the browser through `window.mangaMaker.agent.getDebugSnapshot()`. The debug snapshot must not include API keys or base64 screenshot data.
