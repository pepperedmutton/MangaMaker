...
6. E2E coverage should open the Agent sidebar from the top-right `Inspector` / `Agent` toggle, verify the ribbon does not own that toggle, receive a test-mode plan, show confirmation for risky plans, execute a confirmed command, and show a clear disabled state when configuration is unavailable.
...
13. Harness/context tests must verify that every project page is represented in the lightweight page index, the creator's current page is marked with `isCurrent=true`, the initial harness does not pre-read every page or preload screenshots/assets, on-demand search/read/list/render tool calls work, batch page read/render tools work for small samples, page render tool calls can return a screenshot plus same-page resources, repeated identical tool calls are skipped, exhausted tool budgets pause with Continue/Stop instead of forcing an answer from incomplete evidence, and large inline image data is redacted before prompt construction.
14. Agent Conversation Context tests must verify that messages persist per project and per role when the Agent sidebar is reopened, that switching the active role switches to that role's single session, that the stable `agent-conversation-context.json` record or `GET /__mangamaker__/agent/conversation-context?projectId=...&roleId=...` exposes the same role session for local debugging agents, and that `Clear context` clears only the active role's editable context.
15. Visual budget tests should verify that render tool schemas expose `detail="preview" | "detail"`, that single-page renders can request `crop`, and that prompt guidance tells the model to reduce image count, pixel dimensions, and crop area instead of relying on PNG/JPG format changes.
16. Document-mode tests must verify that default project Markdown documents are created under `docs/`, that `GET /__mangamaker__/agent/documents` and document read/write/delete APIs work, that the left overview remains page-thumbnail-only, that switching to `Docs` first shows a Markdown file-viewer list, that right-click can create a document and rename or delete any document, that deleted default documents are not silently recreated, that selecting a file opens a rendered Markdown viewer, and that explicit edits to a Markdown document persist.
17. Role/metadoc tests must verify that every active role has exactly one `metadocId`, that default roles bind default metadocs, that creating a role can generate a metadoc or bind an ordinary document, that deleting a metadoc removes the role, and that deleting a role keeps the metadoc as an ordinary document.
18. Harness tests must verify that the active role metadoc is available to the model, that role and document tools are available on demand, and that the initial Agent prompt does not preload all document bodies.
19. Role UI tests should verify that the Agent role selector is visible, that it shows each role's metadoc, and that role behavior is documented as assistance, document maintenance, and bounded command-plan preparation rather than autonomous comic authorship.
20. Agent Config UI tests should verify that the creator can edit the system prompt and conversation context, including both user messages and Agent replies, and that the edited values are sent in the next Agent run request.
21. Agent UI tests should verify that tool call status appears inline in the conversation, that no standalone Tool Log panel is rendered, and that clearing Conversation Context does not add a context-delete event to tool logs.
22. Document write tests must verify that `writeDocument` requires `operationId`, that identical retries are idempotent, and that conflicting reuse of an operation id fails clearly.
