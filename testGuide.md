...
12. Harness/context tests must verify that every project page is represented in the lightweight page index, the creator's current page is marked with `isCurrent=true`, the initial harness does not pre-read every page or preload screenshots/assets, on-demand search/read/list/render tool calls work, batch page read/render tools work for small samples, page render tool calls can return a screenshot plus same-page resources, repeated identical tool calls are skipped, exhausted tool budgets produce a limited answer instead of a generic failure, and large inline image data is redacted before prompt construction.
...
14. Visual budget tests should verify that render tool schemas expose `detail="preview" | "detail"`, that single-page renders can request `crop`, and that prompt guidance tells the model to reduce image count, pixel dimensions, and crop area instead of relying on PNG/JPG format changes.
