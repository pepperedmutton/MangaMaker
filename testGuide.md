...
17. Document-mode tests must verify that default project Markdown documents are created under `docs/`, that `GET /__mangamaker__/agent/documents` and document read/write/delete APIs work, that the left overview remains page-thumbnail-only, that switching to `Docs` first shows a Markdown file-viewer list, that right-click can create a document and rename or delete any document, that deleted default documents are not silently recreated, that selecting a file opens a rendered Markdown viewer, and that explicit edits to a Markdown document persist.
18. Role/metadoc tests must verify that every active role has exactly one `metadocId`, that default roles bind default metadocs, that creating a role can generate a metadoc or bind an ordinary document, that deleting a metadoc removes the role, and that deleting a role keeps the metadoc as an ordinary document.
19. Harness tests must verify that the active role metadoc is available to the model, that role and document tools are available on demand, and that the initial Agent prompt does not preload all document bodies.
20. Role UI tests should verify that the Agent role selector is visible, that it shows each role's metadoc, and that role behavior is documented as assistance, document maintenance, and bounded command-plan preparation rather than autonomous comic authorship.
