...
6. E2E coverage should open the Agent sidebar from the top-right `Inspector` / `Agent` toggle, verify the ribbon does not own that toggle, verify test mode can answer and write documents, verify page-edit requests are refused as executable plans, and show a clear disabled state when configuration is unavailable.
7. Legacy multi-command plan tests should remain available for lower-level command infrastructure, but they are not part of the product Agent mutation path while page/canvas edits are disabled.
...
10. Model availability tests must reject Google, Anthropic, OpenAI, non-JSON-capable models, and text-only models other than `deepseek/deepseek-v4-pro`. DeepSeek/Kimi multimodal models may be available only when metadata includes image input, text output, and `response_format`; `deepseek/deepseek-v4-pro` must be available only as text-only document capability with vision disabled.
11. When debugging Agent stalls, verify `GET /__mangamaker__/agent/debug` and `window.mangaMaker.agent.getDebugSnapshot()` report the current busy state, pending tool call, recent tool logs, active run id/status, and sanitized context without exposing API keys or raw base64 screenshots.
12. Agent run tests must verify that `POST /__mangamaker__/agent/runs` creates a durable run, that the run contains `model_request`, `tool_call`, `tool_result`, `model_resume`, `retry`, or `command_plan` steps as applicable, that `GET /__mangamaker__/agent/runs/:runId?projectId=...` can inspect the run, and that `GET /__mangamaker__/agent/runs/:runId/events` can stream live status.
13. Harness/context tests must verify that every project page is represented in the lightweight page index, the creator's current page is marked with `isCurrent=true`, `PrimeDirective.md` is preloaded as project-level direction, the initial harness does not pre-read every page or preload screenshots/assets, on-demand search/read/list/render tool calls work, batch page read/render tools work for small samples, page render tool calls can return a screenshot plus same-page resources, repeated identical tool calls are skipped, exhausted tool budgets pause with Continue/Stop instead of forcing an answer from incomplete evidence, and large inline image data is redacted before prompt construction.
14. Harness compaction tests must verify that the latest unique document/tool result remains visible to the model even after many budget or skipped-tool status results, and that repeated identical tool calls produce a corrective skipped-tool result instead of re-reading the same resource indefinitely.
15. Agent Conversation Context tests must verify that messages persist per project and per role when the Agent sidebar is reopened, that switching the active role switches to that role's single session, that the stable `agent-conversation-context.json` record or `GET /__mangamaker__/agent/conversation-context?projectId=...&roleId=...` exposes the same role session for local debugging agents, and that `Clear context` clears only the active role's editable context.
16. Visual budget tests should verify that render tool schemas expose `detail="preview" | "detail"`, that single-page renders can request `crop`, and that prompt guidance tells the model to reduce image count, pixel dimensions, and crop area instead of relying on PNG/JPG format changes.
17. Document-mode tests must verify that default project Markdown documents are created under `docs/`, that required `docs/PrimeDirective.md` is created/backfilled and cannot be deleted, that `GET /__mangamaker__/agent/documents` and document read/write/delete APIs work, that the left overview remains page-thumbnail-only, that switching to `Docs` first shows a Markdown file-viewer list, that right-click can create a document and rename/delete deletable documents, that deleted default role documents are not silently recreated, that selecting a file opens a rendered Markdown viewer, and that explicit edits to a Markdown document persist.
18. Role/metadoc tests must verify that every active role has exactly one `metadocId` and one working directory, that every role metadoc filename matches the role name under `docs/roles/`, that default roles bind default metadocs, that metadocs are role prompts/definitions rather than output logs, that creating a role asks for the role name plus a working directory path, that legacy role titles are normalized to the name, that creating a role can generate a metadoc or bind an ordinary document, that the backend persists/creates the working directory under `docs/`, that deleting a metadoc removes the role, and that deleting a role keeps the metadoc as an ordinary document.
19. Harness tests must verify that `PrimeDirective.md` and the active role metadoc are pinned model context, that role working directory metadata is present, that role and document tools are available on demand, and that the initial Agent prompt does not preload all other document bodies. Text-only model tests must verify that `PrimeDirective.md` and the active role metadoc are available, Markdown document tools are exposed, and page/image/render tool calls are blocked without leaking resource content.
20. Role UI tests should verify that the Agent role selector is visible, that it shows each role's metadoc and working directory, and that role behavior is documented as assistance and document maintenance rather than autonomous comic authorship or direct page editing.
...
23. Document write tests must verify that `appendDocument`, `replaceDocumentSection`, `replaceDocumentText`, and `writeDocument` require `operationId`, that identical retries are idempotent, and that conflicting reuse of an operation id fails clearly.
